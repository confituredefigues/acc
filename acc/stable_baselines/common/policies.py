...
from stable_baselines.common.tf_layers import conv, linear, conv_to_fc, lstm, minibatchAC, transformer
...
def nature_cnn(scaled_images, **kwargs):
    ...
    """
    activ = tf.nn.elu
    layer_1_pi = activ(conv(scaled_images, 'c1_pi', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_2_pi = activ(conv(layer_1_pi, 'c2_pi', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_3_pi = activ(conv(layer_2_pi, 'c3_pi', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_3_pi = conv_to_fc(layer_3_pi)

    layer_1_vf = activ(conv(scaled_images, 'c1_vf', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_2_vf = activ(conv(layer_1_vf, 'c2_vf', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_3_vf = activ(conv(layer_2_vf, 'c3_vf', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    ...
    # layer_3 = activ(conv(layer_2, 'c3', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), **kwargs))
    # layer_3 = conv_to_fc(layer_3)

    return activ(linear(layer_3_pi, 'fc1_pi', n_hidden=256, init_scale=np.sqrt(2))), activ(linear(layer_3_vf, 'fc1_vf', n_hidden=256, init_scale=np.sqrt(2)))
...
class FeedForwardPolicy_ACC(ActorCriticPolicy):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None, net_arch=None,
        ...
        super(FeedForwardPolicy_ACC, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse,
        ...
        with tf.compat.v1.variable_scope("model", reuse=reuse):
            if feature_extraction == "cnn" and self.pdtype.probability_distribution_class() == CategoricalProbabilityDistribution:
                ...
                self.pi_critic_features_logits = linear(query_value_key_attention, 'pi_critic', ac_space.n, init_scale=1.0, init_bias=0.0)
                ...
                self.pi_critic_features_mean = linear(query_value_key_attention, 'pi_critic', ac_space.shape[0], init_scale=1.0, init_bias=0.0)
                self.pi_critic_features_logstd = tf.compat.v1.get_variable(name='pi_critic/logstd', shape=[1, ac_space.shape[0]], initializer=tf.zeros_initializer())
...
class CnnPolicy_ACC(FeedForwardPolicy_ACC):
    """
    Policy object that implements actor critic, using a CNN (the nature CNN)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(CnnPolicy_ACC, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
                                        feature_extraction="cnn", **_kwargs)


class MlpPolicy_ACC(FeedForwardPolicy_ACC):
    """
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **_kwargs):
        super(MlpPolicy_ACC, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse,
...
_policy_registry = {
    ActorCriticPolicy: {
        "CnnPolicy": CnnPolicy,
        "CnnPolicy_ACC": CnnPolicy_ACC,
        "MlpPolicy": MlpPolicy,
        "MlpPolicy_ACC": MlpPolicy_ACC,
    }
}
